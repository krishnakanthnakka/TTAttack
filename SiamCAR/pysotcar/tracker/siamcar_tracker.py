...
from pysotcar.tracker.base_tracker import SiameseTracker
from pysotcar.utils.misc import bbox_clip
...
class SiamCARTracker(SiameseTracker):
    ...
    def init(self, img, bbox):
        ...
        self.template = z_crop  # ADDED

    def init_adv(self, img, bbox, GAN):
        """
        args:
            img(np.ndarray): BGR image
            bbox: (x, y, w, h) bbox
        ...
        z_crop, perturbmetrics = adv_attack_template(z_crop, GAN)

        self.model.template(z_crop)
        self.template = z_crop  # ADDED
        output_dict = perturbmetrics

        return output_dict

    def init_adv_S(self, img, bbox, GAN):
        """
        args:
            img(np.ndarray): BGR image
            bbox: (x, y, w, h) bbox
        ...
        z_crop, perturbmetrics = adv_attack_template_S(z_crop, GAN)
        output_dict = perturbmetrics
    ...
    def track(self, img, hp):
        ...
        s_c = self.change(self.sz(ave_w, ave_h) /
                          self.sz(self.size[0] * self.scale_z, self.size[1] * self.scale_z))
    ...
    def track_advT(self, img, hp, GAN, dir_, save_path=None, frame_id=None):
        """
        args:
            img(np.ndarray): BGR image
        return:
        ...
        # ADD PERTURBATIONS
        x_crop, perturbmetrics = adv_attack_search_T(x_crop, self.template, GAN, dir_, frame_id)
    ...
    def track_adv(self, img, hp, GAN):
        ...
        # add perturbbations
        x_crop, perturbmetrics = adv_attack_search(x_crop, GAN)
