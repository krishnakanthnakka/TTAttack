...
class AccuracyRobustnessBenchmark_Targeted:
    """
    Args:
        dataset:
        burnin:
    ...
    def __init__(self, dataset, burnin=10, model_epoch=None, expcase=None, trajcase=None):
    ...
    def show_result(self, result, eao_result=None, show_video_level=False, helight_threshold=0.5, log=print):
        """pretty print result
        Args:
        ...
        for tracker_name in tracker_names:
            ...
            if eao_result is None:
                log(formatter.format(tracker_name, accuracy, robustness, lost_number))
            else:
                log(formatter.format(tracker_name, accuracy, robustness,
        ...
        for i in range(len(self.dataset)):
            ...
            if tracker_name not in video.pred_trajs:
                tracker_trajs = video.load_tracker(self.dataset.tracker_path,
                                                   tracker_name, False, self.model_epoch, self.expcase, self.trajcase)


                #print(len(tracker_trajs[0]))
                #exit()
                #tracker_trajs = gt_traj
            else:
