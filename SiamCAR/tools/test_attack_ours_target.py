...
parser.add_argument('--snapshot', type=str, default='snapshot/checkpoint_e20.pth', help='snapshot of models to eval')
...
def load_generator():
        ...
        if  1:
            ...
            attack_method = 'OURS'
            import sys
            sys.path.insert(0, '/cvlabdata1/home/krishna/AttTracker/pix2pix')
            ...
            ckpt = os.path.join('/cvlabdata1/home/krishna/AttTracker',
                                'checkpoints/{}_{}/{}'.format(opt.model, expcase, model_epoch))
            print(" Loading generator trained with OURS approach")

        # CSA-S
        elif 0:
            opt.model, expcase, model_epoch, opt.netG = 'G_search_L2_500_regress', 3, '1_net_G.pth', 'unet_256'
            ckpt = os.path.join('/cvlabdata1/home/krishna/AttTracker/GIT/CSA/',
                            'checkpoints/{}_{}/{}'.format(opt.model, expcase, model_epoch))


        # CSA-T
        elif 0:
            opt.model, expcase, model_epoch, opt.netG = 'G_template_L2_500_regress', 3, '1_net_G.pth', 'unet_128'
            ckpt = os.path.join('/cvlabdata1/home/krishna/AttTracker/GIT/CSA/',
                            'checkpoints/{}_{}/{}'.format(opt.model, expcase, model_epoch))
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    if idx == 0 and args.vis:
                        ...
                        w, h = img.shape[:2]
                        video_out = cv2.VideoWriter(os.path.join("./viz/", video.name + ".avi"), fourcc, fps=20, frameSize=(h, w))
                ...
                if args.vis and idx > frame_counter:
                    ...
                    cv2.rectangle(img, (target_bbox_rect[0], target_bbox_rect[1]), (target_bbox_rect[0] + target_bbox_rect[2], target_bbox_rect[1] + target_bbox_rect[3]), (0, 0, 255), 3)
            ...
            if args.attack_universal:
                video_path = os.path.join('results_U_{}_{}'.format(attack_method, expcase), args.dataset, str(args.targetcase), model_name,'baseline', video.name)
            else:
                video_path = os.path.join('results_{}_{}'.format(attack_method, expcase), args.dataset, str(args.targetcase), model_name,'baseline', video.name)
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if args.vis and idx > 0:
                    if not any(map(math.isnan,gt_bbox)):
                        ...
                        cv2.putText(img, str(idx), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                        cv2.rectangle(img, (target_bbox[0], target_bbox[1]), (target_bbox[0] + target_bbox[2], target_bbox[1] + target_bbox[3]), (0, 0, 255), thickness)
            ...
            if args.attack_universal:
                ...
                model_path = os.path.join('results_{}_{}'.format(attack_method, expcase), args.dataset,  str(args.targetcase),model_name)
