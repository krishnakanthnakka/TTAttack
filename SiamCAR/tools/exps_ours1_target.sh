for ((TRAJ=11;TRAJ<=14;TRAJ++)); do
    python test_attack_ours_target.py  --dataset OTB100  --snapshot snapshot/model_general.pth   --model_iter=4_net_G.pth --case=211 --eps=16 --attack_universal --trajcase=$TRAJ  --targetcase=$TRAJ  --directions=8 #--vis
done
exit 1


for ((CASE=320;CASE<=339;CASE++)); do
    python test_attack_ours_target.py  --dataset OTB100  --snapshot snapshot/model_general.pth   --model_iter=4_net_G.pth --case=$CASE --eps=16 --attack_universal --trajcase=11  --targetcase=11 #--vis
done
