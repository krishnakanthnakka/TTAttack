...
# python test_attack_iou.py   --dataset VOT2018 --snapshot snapshot/model_general.pth
...
#from pysotcar.tracker.tracker_builder import build_tracker
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    last_gt = save_bbox
                    # heavy noise image
                    heavy_noise = np.random.randint(-1, 2,
                                                    (image.shape[0], image.shape[1], image.shape[2])) * 128
                    ...
                    # query
                    outputs_orig = tracker.track_fixed(img, hp)
                    outputs_target = tracker.track_fixed(image_noise, hp)
                    ...
                    if target_score < 0.8:
                        ...
                        while True:
                            ...
                            outputs_adv = tracker.track_fixed((trial_sample + 128).astype(np.uint8), hp)
                            # IoU score
                            ...
                            while True:
                                ...
                                for i in np.arange(10):
                                    ...
                                    outputs_adv = tracker.track_fixed(
                                        (trial_sample + 128).astype(np.uint8), hp)
                                    # IoU score
            ...
            # save results
            video_path = os.path.join('results_iou', args.dataset, model_name,
                                      'baseline', video.name)
            ...
            print('({:3d}) Video: {:12s} Time: {:4.1f}s mean Speed: {:3.1f}fps Speed: {:3.1f}fps Lost: {:d}'.format(
                v_idx + 1, video.name, toc, np.mean(mean_FPS), idx / toc, lost_number))
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == 0:
                    ...
                    image_noise = image_noise.astype(np.uint8)
                    # query
                    outputs_orig = tracker.track_fixed(image, hp)
                    outputs_target = tracker.track_fixed(image_noise, hp)
                    target_score = overlap_ratio(
                        np.array(outputs_orig['bbox']), np.array(outputs_target['bbox']))
                    adversarial_sample = image.astype(np.float) - 128
            ...
            if 'VOT2018-LT' == args.dataset:
                ...
                model_path = os.path.join('results_iou', args.dataset, model_name)
