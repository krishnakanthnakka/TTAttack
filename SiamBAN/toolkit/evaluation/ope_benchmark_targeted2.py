...
class OPEBenchmark_Targeted2:
    ...
    def __init__(self, dataset, trajcase):
    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                gt_traj_file = os.path.join(self.dataset.tracker_path, str(self.trajcase),tracker_name, 'baseline',
                                             video.name, video.name + '_001_target2.txt')
                ...
                if tracker_name not in video.pred_trajs:
                    tracker_traj = video.load_tracker(self.dataset.tracker_path+"/{}".format(str(self.trajcase)),
                                                      tracker_name, False, trajcase=self.trajcase)
                    tracker_traj = np.array(tracker_traj[0])
                    #tracker_traj = np.array([ np.array(x) for x in tracker_traj])

                    # print("YO", tracker_traj[0])
                    # exit()
                ...
                #print(tracker_traj.shape, gt_traj.shape)
