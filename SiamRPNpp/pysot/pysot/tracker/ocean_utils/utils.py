...
def load_yaml(path, subset=True):
    ...
    path = path.split("/")[-4:]
    path = "/".join([f for f in path])
    # file = open(os.path.join(
    #     '/cvlabdata1/home/krishna/AttTracker/pysot/pysot/tracker/ocean_utils', path), 'r')

    file = open(os.path.join(root_dir, path), 'r')
...
def get_subwindow_tracking_mask(im, pos, model_sz, original_sz, out_mode='torch'):
    ...
    crop_info['crop_cords'] = [context_xmin,
                               context_xmax, context_ymin, context_ymax]
...
def generate_anchor(total_stride, scales, ratios, score_size):
    ...
    # this order is right  [5, 17, 17, 4]
    anchor = np.reshape(anchor, (5, score_size, score_size, 4))
...
def crop_with_boxes(img_tensor, x_crop_boxes, out_height, out_width, crop_inds=None, avg_channels=True,
    ...
    if isinstance(x_crop_boxes, list):
        crop_boxes = torch.tensor(
            x_crop_boxes, dtype=torch.float32).to(img_device)
        ...
        raise ValueError(
            'Unknown type for crop_boxes {}'.format(type(x_crop_boxes)))
    ...
    if crop_inds is not None:
        if isinstance(crop_inds, list) or isinstance(crop_inds, np.ndarray):
            crop_inds = torch.tensor(
                crop_inds, dtype=torch.float32).to(img_device)
        elif isinstance(crop_inds, torch.Tensor):
            crop_inds = crop_inds.to(device=img_device, dtype=torch.float32)
        else:
            raise ValueError(
                'Unknown type for crop_inds {}'.format(type(crop_inds)))
        crop_inds = crop_inds.view(-1)
        assert crop_inds.size(0) == crop_boxes.size(0)
    else:
        if num_imgs == 1:
            crop_inds = torch.zeros(
                num_crops, dtype=torch.float32, device=img_device)
        elif num_imgs == num_crops:
            crop_inds = torch.arange(
                num_crops, dtype=torch.float32, device=img_device)
        else:
            raise ValueError('crop_inds MUST NOT be None.')

    if avg_channels:
        img_channel_avg = img_tensor.mean(
...
def load_dataset(dataset):
    ...
    if 'OTB' in dataset:
        ...
        json_path = join(realpath(dirname(__file__)),
        ...
        for video in videos:
            ...
            gt_path = join(video_path, 'groundtruth.txt')
            gt = np.loadtxt(gt_path, delimiter=',').astype(np.float64)
            info[video] = {'infrared_imgs': in_image_files,
                           'visiable_imgs': rgb_image_files, 'gt': gt, 'name': video}
        ...
        for video in videos:
            ...
            gt_path = join(video_path, 'groundtruth.txt')
            gt = np.loadtxt(gt_path, delimiter=',')
            info[video] = {'image_files': image_files,
                           'gt': [gt], 'name': video}
        ...
        for f_video in father_videos:
            ...
            for s_video in son_videos:
                ...
                info[s_video] = {'image_files': image_files,
                                 'gt': gt, 'name': s_video}
        ...
        base_path = join(realpath(dirname(__file__)),
                         '../../dataset', 'YTBVOS', 'valid')
        json_path = join(realpath(dirname(__file__)),
                         '../../dataset', 'YTBVOS', 'valid', 'meta.json')
        ...
        for v in meta.keys():
            ...
            info[v]['anno_files'] = [
                join(base_path, 'Annotations', v, im_f + '.png') for im_f in frames]
            info[v]['anno_init_files'] = [
                join(base_path, 'Annotations', v, im_f + '.png') for im_f in anno_frames]
            info[v]['image_files'] = [
                join(base_path, 'JPEGImages', v, im_f + '.jpg') for im_f in frames]
            info[v]['name'] = v
        ...
        raise ValueError(
            "Dataset not support now, edit for other dataset youself...")
...
def load_pretrain(model, pretrained_path, print_unuse=True):
    print('Yo! load pretrained model from {}'.format(pretrained_path))
    ...
    if "state_dict" in pretrained_dict.keys():
        ...
        pretrained_dict = remove_prefix(
            pretrained_dict, 'feature_extractor.')  # remove online train
    else:
        pretrained_dict = remove_prefix(
            pretrained_dict, 'module.')  # remove multi-gpu label
        pretrained_dict = remove_prefix(
            pretrained_dict, 'feature_extractor.')   # remove online train
...
def aug_apply(bbox, param, shape, inv=False, rd=False):
    ...
    if not inv:
        ...
        if 'scale' in param:
            ...
            center = Center(center.x, center.y, center.w
                            * scale_x, center.h * scale_y)
        ...
        center = Center(center.x - tx, center.y - ty,
                        center.w / scale_x, center.h / scale_y)
...
def print_speed(i, i_time, n, logger):
    ...
    # for philly. let's reduce it in case others kill our job 100-25
    logger.info('\nPROGRESS: {:.2f}%\n'.format(100 * i / n))
