...
from models_ocean.online.layers.blocks import conv_block
from models_ocean.online.external.PreciseRoIPooling.pytorch.prroi_pool import PrRoIPool2D
import math
...
class FilterPool(nn.Module):
    ...
    def __init__(self, filter_size=1, feature_stride=16, pool_square=False):
        super().__init__()
        self.prroi_pool = PrRoIPool2D(
            filter_size, filter_size, 1 / feature_stride)
    ...
    def forward(self, feat, bb):
        ...
        batch_index = torch.arange(
            num_images_total, dtype=torch.float32).view(-1, 1).to(bb.device)
...
class FilterInitializer(nn.Module):
    ...
    def __init__(self, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
        ...
        self.filter_pool = FilterPool(
            filter_size=filter_size, feature_stride=feature_stride, pool_square=pool_square)
        ...
        for i in range(num_filter_post_convs):
            post_conv_layers.append(conv_block(
                feature_dim, feature_dim, kernel_size=1, padding=0))
    ...
    def forward(self, feat, bb):
        ...
        if num_images > 1:
            weights = torch.mean(weights.view(
...
class FilterInitializerLinear(nn.Module):
    ...
    def __init__(self, filter_size=1, feature_dim=256, feature_stride=16, pool_square=False, filter_norm=True,
        ...
        self.filter_conv = nn.Conv2d(
            feature_dim, feature_dim, kernel_size=conv_ksz, padding=conv_ksz // 2)
        self.filter_pool = FilterPool(
            filter_size=filter_size, feature_stride=feature_stride, pool_square=pool_square)
        self.filter_norm = filter_norm
