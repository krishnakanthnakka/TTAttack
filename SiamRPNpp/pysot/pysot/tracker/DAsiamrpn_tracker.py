...
class DASiamRPNTracker(SiameseTracker):
    def __init__(self, model, dataset):
        super(DASiamRPNTracker, self).__init__()
    ...
    def init(self, im, bbox):
        ...
        if self.cfg.TRACK.ADAPTIVE:
            if ((target_sz[0] * target_sz[1]) / float(state['im_h'] * state['im_w'])) < 0.004:
                self.INSTANCE_SIZE = 287  # small object big search region
            else:
    ...
    def track_advT(self, im, GAN, dir_):
        ...
        d_search = (self.INSTANCE_SIZE - self.cfg.TRACK.EXEMPLAR_SIZE) / 2
        pad = d_search / scale_z
        s_x = s_z + 2 * pad

        # extract scaled crops for search region x at previous target position
        x_crop = Variable(get_subwindow_tracking(
            im, target_pos, self.INSTANCE_SIZE, round(s_x), avg_chans).unsqueeze(0))

        x_crop, perturbmetrics = adv_attack_search_T(x_crop.cuda(), self.template, GAN, dir_)
