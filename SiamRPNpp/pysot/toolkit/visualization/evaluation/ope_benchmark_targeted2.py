    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                gt_traj_file = os.path.join(self.dataset.tracker_path, tracker_name, 'baseline',
                                            str(self.expcase), self.model_epoch, str(self.trajcase), video.name, video.name + '_001_target2.txt')
    ...
    def eval_norm_precision(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if tracker_name not in video.pred_trajs:
                    tracker_traj = video.load_tracker(self.dataset.tracker_path,
                                                      tracker_name, False, self.model_epoch, self.expcase)
                    tracker_traj = np.array(tracker_traj)[0]
                else:
                    tracker_traj = np.array(video.pred_trajs[tracker_name])
                n_frame = len(gt_traj)
