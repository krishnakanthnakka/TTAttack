...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        self.img_names = [x.replace("/color", "/") for x in self.img_names]
        self.imgs = None
    ...
    def load_tracker(self, path, tracker_names=None, store=True, model_epoch=None, expcase=None, trajcase=None):
        ...
        for name in tracker_names:
            ...
            if trajcase is None:
                ...
                # CHANGED. for specific case in dimp. comment out for general
                #traj_file = os.path.join(path, name, str(expcase), model_epoch, 'uav_'+self.name + '.txt')



            else:
                traj_file = os.path.join(path, name, str(
                    expcase), model_epoch, str(trajcase), self.name + '.txt')
