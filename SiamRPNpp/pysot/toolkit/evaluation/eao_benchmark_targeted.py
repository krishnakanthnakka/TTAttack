...
class EAOBenchmark_Targeted:
    ...
    def __init__(self, dataset, skipping=5, tags=['all', 'camera_motion', 'illum_change', 'motion_change',
                                                  'size_change', 'occlusion', 'empty'], model_epoch=None, expcase=None, trajcase=None):
    ...
    def show_result(self, result, topk=10, log=print):
        """pretty print result
        Args:
        ...
        if len(self.tags) == 1:
            ...
            for tracker_name, eao in tracker_eao:
                ...
                log(formatter.format(tracker_name, eao))
            log(bar)
        else:
            ...
            for tracker_name in sorted_tacker:
                # for tracker_name, ret in result.items():
                log("|{:^20}|".format(tracker_name) + formatter.format(
