...
python run_tsne_utils.py   --tracker_name=siamrpn_r50_l234_dwxcorr --dataset=OTB100 --case=54 --gpu=1 --model_iter=4_net_G.pth --attack_universal
...
python run_template_adv2.py   --tracker_name=siamrpn_mobilev2_l234_dwxcorr --dataset=OTB100 --case=54 --gpu=1 --model_iter=4_net_G.pth --attack_universal
...
def main(cmd_line):
   ...
   # exit()
    ...
    log("Case: {}\nEpsilon: {}\nTracker: {}\nCheckpoint iter: {}\n".format(args.case, opt.eps, args.tracker_name, args.model_iter))
    st_time = time.time()
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in (enumerate(video)):
                ...
                if idx == 0:
                    ...
                    if args.vis:
                        fourcc = cv2.VideoWriter_fourcc(*'XVID')
                        w, h = img.shape[:2]
                        print(os.path.join(savedir2, video.name + ".avi"))
                        video_out = cv2.VideoWriter(os.path.join(savedir2, video.name + ".avi"), fourcc, fps=20, frameSize=(h, w))
                        video_search = cv2.VideoWriter(os.path.join(savedir2, video.name + "_search.avi"), fourcc, fps=20, frameSize=(255, 255))
                        video_perturb = cv2.VideoWriter(os.path.join(savedir2, video.name + "_perturb.avi"), fourcc, fps=20, frameSize=(255, 255))
                    ...
                    x_feats.append(outputs['xf'].detach().cpu().numpy())
                    z_feat = outputs['zf']

                    # print(outputs['xf'].shape)
                    # exit()
            ...
            x_feats = np.concatenate(x_feats, 0)
            print(x_feats.shape)

            feats['x_feats'] = x_feats
            feats['z_feat'] = z_feat

            # Store data (serialize)
            with open('./misc2/feats_dict.pickle', 'wb') as handle:
