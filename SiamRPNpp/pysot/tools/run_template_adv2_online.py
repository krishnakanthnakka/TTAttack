...
python run_template_adv2_online.py --dataset=OTB100  --tracker_name=siam_ocean_online --case=54  --model_iter=4_net_G.pth --gpu=0 --trajcase=0 --attack_universal
python run_template_adv2_online.py --dataset=OTB100  --tracker_name=siam_ocean_online --case=54  --model_iter=4_net_G.pth --gpu=0 --trajcase=0


python run_template_adv2_online.py --dataset=lasot  --tracker_name=siam_ocean_online --case=54  --model_iter=5_net_G.pth --gpu=0 --trajcase=0
...
def main(cmd_line):
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in tqdm(enumerate(video)):
                ...
                if idx == 0:
                    ...
                    online_tracker.init_adv_T(img, tracker.siam_net, gt_bbox_, load_online=True, dataname=args.dataset,
                        resume='snapshot/{}'.format(ckpt_dict[args.dataset]))
                    ...
                    outputs = online_tracker.track_advT(img, state, GAN, tracker, idx, direction=1)
            ...
            log('({:3d}) Video: {:12s} Time: {:5.1f}s Speed: {:3.1f}fps, mean Speed: {:3.1f}, MAE: {:2.1f}, Avg. Mean: {:2.1f}, Lost:{:4d}'.format(
                v_idx + 1, video.name, toc, idx / toc,  np.mean(mean_FPS),  np.mean(MAE), np.mean(MAE_), outputs['lost']))
