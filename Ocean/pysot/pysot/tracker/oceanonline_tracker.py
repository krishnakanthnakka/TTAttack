...
from .ocean_utils.utils import TrackerParams, FeatureParams, load_yaml, load_pretrain
from .ocean_utils.online import dcf, TensorList, augmentation, operation, fourier
from .ocean_utils.online.preprocessing import numpy_to_torch, sample_patch_multiscale, sample_patch_transformed

from .ocean_utils.models_ocean import models as models
import torch
...
class OceanOnlineTracker(object):
    """
    modified from DiMP released
    """

    def __init__(self, model=None, dataset=None, checkpoint_name='', online=False):
        super(OceanOnlineTracker, self).__init__()

        siam_info = edict()
        siam_info.arch = 'Ocean'
        siam_info.dataset = dataset
        siam_info.online = online
        ...
        # self.online_resume = os.path.join(root_dir, 'ocean_utils', 'snapshot', '{}'.format(
        #     checkpoint_name))

        self.online_resume = os.path.join(root_dir, '../../../../tracker_weights/siam_ocean_online', '{}'.format(
            checkpoint_name))

        # '/cvlabdata1/home/krishna/AttTracker/pysot/pysot/tracker/ocean_utils/snapshot/{}'.format(
        #     checkpoint_name)
    ...
    def init(self, image, siam_net, bbox, load_online, dataname='VOT2019', resume=None):
        ...
        #     '/cvlabdata1/home/krishna/AttTracker/pysot/pysot/tracker/ocean_utils', resume)

        resume_path = os.path.join(
            '/cvlabdata1/home/krishna/AttTracker/pysot/pysot/tracker/ocean_utils', resume)

        self.params.net = load_pretrain(
            self.params.net, resume_path, print_unuse=False)
        ...
        self.pos = torch.Tensor(
            [target_pos[1], target_pos[0]])   # height, width
        ...
        search_area = torch.prod(
            self.target_sz * self.p.search_area_scale).item()
    ...
    def init_adv_T(self, image, siam_net, bbox, load_online=True, dataname='VOT2019', resume=None):
        # Initialize some stuff
        ...
        #resume_path = os.path.join(root_dir, 'ocean_utils', resume)

        resume_path = os.path.join(root_dir, '../../../../tracker_weights/siam_ocean_online', resume)
    ...
    def track(self, image, siam_state, GAN, siam_tracker) -> dict:
        self.debug_info = {}
        bgr_im = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        ...
        # print(scores_raw.shape, test_x.shape)
        # exit()

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(
            scores_raw, sample_scales)

        # self.siam_state = siam_tracker.track(siam_state, bgr_im, s)

        siam_out = siam_tracker.track(bgr_im, siam_state, s)
        self.siam_state = siam_out
        ...
        if flag != 'not_found':
            if getattr(self.p, 'use_iou_net', True):
                update_scale_flag = getattr(
                    self.p, 'update_scale_when_uncertain', True) or flag != 'uncertain'
        ...
        learning_rate = getattr(
            self.p, 'hard_negative_learning_rate', None) if hard_negative else None
        ...
        outputs = {}
        outputs['state'] = siam_state

        target_pos = siam_state['target_pos']
        target_sz = siam_state['target_sz']

        outputs['target_pos'] = siam_state['target_pos']
        outputs['target_sz'] = siam_state['target_sz']
        outputs['lost'] = 0
        outputs['best_score'] = 1.0

        outputs['bbox'] = [target_pos[0] - target_sz[0] / 2,
                           target_pos[1] - target_sz[1] / 2,
                           target_sz[0],
                           target_sz[1]]
        outputs['cropx'] = siam_out['cropx']
        outputs['lost'] = 0
        outputs['metrics'] = siam_out['metrics']

        return outputs

    def track_advT(self, image, siam_state, GAN, siam_tracker, idx, direction) -> dict:
        ...
        # siam_State variable is updated in the following below line
        siam_out = siam_tracker.track_advT(bgr_im, GAN=GAN, dir_=direction,
                                           frame_index=idx, state=siam_state, online_score=s)
        self.siam_state = siam_out
    ...
    def localize_target(self, scores, sample_scales):
        ...
        translation_vec = target_disp * \
            (self.img_support_sz / self.feature_sz) * sample_scales[scale_ind]
    ...
    def localize_advanced(self, scores, sample_scales):
        ...
        translation_vec1 = target_disp1 * \
            (self.img_support_sz / self.feature_sz) * sample_scale

        if max_score1.item() < self.p.target_not_found_threshold:
            return translation_vec1, scale_ind, scores_hn, 'not_found'

        # Mask out target neighborhood
        target_neigh_sz = self.p.target_neighborhood_scale * \
            (self.target_sz / sample_scale) * \
        ...
        if max_score2 > self.p.distractor_threshold * max_score1:
            ...
            disp_threshold = self.p.dispalcement_scale * \
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(
            im, pos, scales, sz, getattr(self.p, 'border_mode', 'replicate'))
        with torch.no_grad():
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        aug_expansion_factor = getattr(
            self.p, 'augmentation_expansion_factor', None)
        ...
        augs = self.p.augmentation if getattr(
            self.p, 'use_augmentation', True) else {}
    ...
    def init_target_boxes(self):
        ...
        init_target_boxes = torch.cat(
            init_target_boxes.view(1, 4), 0).to(self.p.device)
    ...
    def update_sample_weights(self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, learning_rate=None):
        ...
        for sw, prev_ind, num_samp, num_init in zip(sample_weights, previous_replace_ind, num_stored_samples, num_init_samples):
            ...
            init_samp_weight = getattr(
                self.p, 'init_samples_minimum_weight', None)
    ...
    def update_state(self, new_pos, new_scale=None):
        ...
        if new_scale is not None:
            ...
            self.target_sz = torch.from_numpy(
                self.target_sz.astype(np.float32).copy())
        ...
        self.pos = torch.max(
            torch.min(torch.from_numpy(new_pos.astype(
                np.float32).copy()), self.image_sz - inside_offset),
            inside_offset)
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if getattr(self.p, 'window_output', False):
            if getattr(self.p, 'use_clipped_window', False):
                self.output_window = dcf.hann2d_clipped(self.output_sz.long(), self.output_sz.long(
                ) * self.p.effective_search_area / self.p.search_area_scale, centered=False).to(self.p.device)
            else:
                self.output_window = dcf.hann2d(
                    self.output_sz.long(), centered=True).to(self.p.device)
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True):
        ...
        if self.p.num_init_random_boxes > 0:
            ...
            rand_factor = square_box_sz * \
                torch.cat([self.p.box_jitter_pos * torch.ones(2),
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        if isinstance(step_length, (tuple, list)):
            step_length = torch.Tensor(
                [step_length[0], step_length[0], step_length[1], step_length[1]], device=self.p.device).view(1, 1, 4)
    ...
    def params_convert(self):
        ...
        self.p.augmentation['relativeshift'] = eval(
            self.p.augmentation['relativeshift'])
