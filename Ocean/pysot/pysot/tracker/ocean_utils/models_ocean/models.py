...
class Ocean(Ocean_):
    def __init__(self, align=False, online=False):
        ...
        self.connect_model = box_tower(
            inchannels=256, outchannels=256, towernum=4)
...
class OninleRes18(MultiFeatureBase):
    ...
    def initialize(self, siam_net):
        ...
        self.layer_stride = {'conv1': 2, 'layer1': 4, 'layer2': 8,
                             'layer3': 16, 'layer4': 32, 'classification': 16, 'fc': None}
        self.layer_dim = {'conv1': 64, 'layer1': 64, 'layer2': 128,
                          'layer3': 256, 'layer4': 512, 'classification': 256, 'fc': None}
...
class ONLINEnet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        train_feat = self.extract_backbone_features(
            train_imgs.view(-1, *train_imgs.shape[-3:]))
        test_feat = self.extract_backbone_features(
            test_imgs.view(-1, *test_imgs.shape[-3:]))
...
def ONLINEnet50(filter_size=4, optim_iter=5, optim_init_step=0.9, optim_init_reg=0.1,
    ...
    # ONLINE network
    net = ONLINEnet(feature_extractor=backbone_net, classifier=classifier,
