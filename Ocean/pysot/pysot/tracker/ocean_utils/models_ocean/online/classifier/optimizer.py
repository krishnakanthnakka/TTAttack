...
import models_ocean.online.layers.filter as filter_layer
import models_ocean.online.layers.activation as activation
from models_ocean.online.layers.distance import DistanceMap
import math
...
class ONLINESteepestDescentGN(nn.Module):
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        center = ((bb[..., :2] + bb[..., 2:] / 2) /
                  self.feat_stride).view(-1, 2).flip((1,)) - dmap_offset
        ...
        for i in range(num_iter):
            ...
            alpha_num = (weights_grad *
                         weights_grad).view(num_sequences, -1).sum(dim=1)
            alpha_den = ((scores_grad * scores_grad).view(num_images, num_sequences,
                                                          - 1).sum(dim=(0, 2)) + reg_weight * alpha_num).clamp(1e-8)
        ...
        if compute_losses:
            ...
            losses.append((((sample_weight * (scores - label_map)) **
                            2).sum() + reg_weight * (weights**2).sum()) / num_sequences)
