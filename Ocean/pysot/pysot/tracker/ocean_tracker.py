...
from .ocean_utils.utils import load_yaml, im_to_torch, get_subwindow_tracking, make_scale_pyramid, python2round, load_pretrain
from .ocean_utils.models_ocean import models
...
from .ocean_utils import ONLINE
...
class OceanTracker(object):
    def __init__(self, model=None, dataset=None, checkpoint_name='', online=False):
        super(OceanTracker, self).__init__()

        siam_info = edict()
        siam_info.arch = 'Ocean'
        siam_info.dataset = dataset  # Set this.
        siam_info.online = online    # Set this. CHANGED
        ...
        siam_net = models.__dict__[siam_info.arch](align=siam_info.align, online=siam_info.online)
        siam_net.eval()
        siam_net = siam_net.cuda()

        # check. CHANGED
        # load_pretrain(
        #     siam_net, os.path.join(root_dir, 'ocean_utils', 'snapshot', '{}'.format(checkpoint_name)))

        load_pretrain(
            siam_net, os.path.join(root_dir, '../../../../tracker_weights/siam_ocean_online', '{}'.format(checkpoint_name)))

        self.siam_net = siam_net

    def init(self, im, bbox, hp=None):
        ...
        if not hp and not self.info.epoch_test:
            prefix = [x for x in ['OTB100', 'VOT', 'GOT10K', 'LASOT']
            ...
            yamlPath = os.path.join(
                absPath, '../../experiments/test/{0}/'.format(prefix[0]), yname)
        ...
        z_crop, _ = get_subwindow_tracking(
            im, target_pos, p.exemplar_size, s_z, avg_chans)
    ...
    def init_adv_T(self, im, bbox, GAN, recompute=False, hp=None):
       ...
       # print(bbox)
        ...
        if not hp and not self.info.epoch_test:
            ...
            # print(prefix, self.info.dataset)
            # exit()
            absPath = os.path.abspath(os.path.dirname(__file__))
            yname = 'Ocean.yaml'
        ...
        return tensor2img(z), state

    def init_adv(self, im, bbox, GAN, hp=None):
       ...
       # print(bbox)
        ...
        '''Adversarial Attack'''
        z_crop = adv_attack_template(z_crop, GAN)
    ...
    def update(self, net, x_crops, target_pos, target_sz, window, scale_z, p):
        ...
        # size penalty
        s_c = self.change(self.sz(pred_x2 - pred_x1, pred_y2 -
                                  - pred_y1) / (self.sz_wh(target_sz)))  # scale penalty
        r_c = self.change((target_sz[0] / target_sz[1]) /
        ...
        if self.online_score is not None:
            ...
            o_score = cv2.resize(
                self.online_score, (s_size, s_size), interpolation=cv2.INTER_CUBIC)
        ...
        diff_xs, diff_ys, pred_w, pred_h = diff_xs / scale_z, diff_ys / \
            scale_z, pred_w / scale_z, pred_h / scale_z
    ...
    #     d_search = (p.instance_size - p.exemplar_size) / \
    ...
    #         net, x_crop.cuda(), target_pos, target_sz * scale_z, window, scale_z, p)
    ...
    def track_advT(self, im, GAN, dir_, frame_index, state=None, online_score=None, gt=None):
        ...
        # print("EXXITED FROM OCEANTRAKER.py")
        # exit()
        ...
        # print(dir_, frame_index)
        # exit()

        x_crop, perturbmetrics = adv_attack_search_T(
            x_crop.cuda(), self.template, GAN, dir_, frame_index)
    ...
    def track(self, im, state=None, online_score=None, gt=None):
...
class OceanConfig(object):
    ...
    instance_size = 255
    total_stride = 8
    score_size = (instance_size - exemplar_size) // total_stride + \
        1 + 8  # for ++
    context_amount = 0.5
    ratio = 0.94
    ...
    def renew(self):
        self.score_size = (
            self.instance_size - self.exemplar_size) // self.total_stride + 1 + 8  # for ++
