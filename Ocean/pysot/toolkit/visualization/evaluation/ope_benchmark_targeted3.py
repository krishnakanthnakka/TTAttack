...
class OPEBenchmark_Targeted3:
    ...
    def __init__(self, dataset, model_epoch, expcase, trajcase):
        self.dataset = dataset
        self.model_epoch = model_epoch
        self.expcase = expcase
        self.trajcase = trajcase
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if  WITHGT:
                    ...
                    #gt_traj = np.array([ j for x in video.gt_traj for j in x])
                    #gt_traj = gt_traj.reshape((-1,4))
                    #print(len (gt_traj), gt_traj)
                # print(video.name)

                else:
                    gt_traj_file = os.path.join(self.dataset.tracker_path, tracker_name, 'longterm',
                                                str(self.expcase), self.model_epoch, str(self.trajcase), video.name, video.name + '_001_target.txt')

                    with open(gt_traj_file, 'r') as f:
                        gt_traj = [list(map(float, x.strip().split(',')))
                                   for x in f.readlines()]

                    gt_traj = np.array(gt_traj)

                #print(gt_traj[0], len(gt_traj))
                # exit()
                #print("\n\n", gt_traj)

                if tracker_name not in video.pred_trajs:
                    tracker_traj = video.load_tracker(self.dataset.tracker_path,
                                                      tracker_name, False, self.model_epoch, self.expcase, str(self.trajcase))

                    tracker_traj = np.array(tracker_traj[0])
                    #print(tracker_traj[0], len(tracker_traj), tracker_traj)
                    # exit()

                    #print("\n\n",len(tracker_traj), tracker_traj[0])

                    #tracker_traj = np.array([ (x) for x in tracker_traj])

                   #print(len(tracker_traj), tracker_traj)
                    tracker_traj[0] = tracker_traj[1]
                    #print("\n\n", len(tracker_traj), tracker_traj)

                else:
                    tracker_traj = np.array(video.pred_trajs[tracker_name])
                n_frame = len(gt_traj)

                #print("Yo", video.name, n_frame, len(tracker_traj))
                ...
                #print("\n\n", len(gt_traj), len(tracker_traj), "\n\n")

                success_ret_[video.name] = success_overlap(gt_traj, tracker_traj, n_frame)

                #print(video.name, success_ret_[video.name])
    ...
    def eval_success_bin(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if tracker_name not in video.pred_trajs:
                    tracker_traj = video.load_tracker(self.dataset.tracker_path,
                                                      tracker_name, False, self.model_epoch)
    ...
    def show_result(self, success_ret, precision_ret=None,
                    norm_precision_ret=None, show_video_level=False, helight_threshold=0.6, log=print):
        """pretty print result
        Args:
        ...
        for tracker_name in tracker_names:
            ...
            if precision_ret is not None:
                precision = np.mean(list(precision_ret[tracker_name].values()), axis=0)[50]
            else:
                precision = 0
            if norm_precision_ret is not None:
                ...
                norm_precision = 0
            log(formatter.format(tracker_name, success, norm_precision, precision))
        log('-' * len(header))
