...
def draw_success_precision(success_ret, name, videos, attr, precision_ret=None,
                           norm_precision_ret=None, bold_name=None, axis=[0, 1], ax=None):
    ...
    if attr == 'ALL':
        ...
        #plt.title(r"$\bf{Success \ plots \ of\ OPE\ on\ UAV123}$")
        plt.title(r"$\bf{Success \ plots \ of\ OPE\ on\ %s}$" % (name))
    ...
    # exit()
    # plt.show()
    #fig.savefig('success_{}.pdf'.format(name), format='pdf', bbox_inches="tight", pad_inches=0.2)
    # plt.savefig('success.png')
    fig.savefig('success_{}.png'.format(name), dpi=300, bbox_inches="tight")
    ...
    if precision_ret:
        ...
        if attr == 'ALL':
            ...
            plt.title(r"$\bf{Precision \ plots \ of\ OPE\ on\ %s}$" % (name))
        ...
        for idx, (tracker_name, pre) in \
            ...
            if tracker_name == bold_name:
                label = r"\textbf{[%.2f] %s}" % (pre, tracker_name)
            else:
                label = "[%.2f] " % (pre) + tracker_name
            value = [v for k, v in precision_ret[tracker_name].items() if k in videos]
            plt.plot(thresholds, np.mean(value, axis=0),
                     color=COLOR[idx], linestyle=LINE_STYLE[idx], label=label, linewidth=2)
        ax.legend(loc='upper left', labelspacing=0.1, fontsize=7)
        ax.autoscale(enable=True, axis='both', tight=True)
        xmin, xmax, ymin, ymax = plt.axis()
        ax.autoscale(enable=False)
        ymax += 0.03
        ...
        ax.set_aspect((xmax - xmin) / (ymax - ymin))
        # plt.show()
        #fig.savefig('precision_{}.pdf'.format(name), format='pdf', bbox_inches="tight", pad_inches=0.2)
        fig.savefig('precision_{}.png'.format(name), dpi=300, bbox_inches="tight")
    ...
    if norm_precision_ret:
        ...
        if attr == 'ALL':
            ...
            plt.title(r"$\bf{ Normalized \ Precision \ plots \ of\ OPE\ on\ %s}$" % (name))
        ...
        ax.set_aspect((xmax - xmin) / (ymax - ymin))
        # plt.show()
        fig.savefig('norm_precision.pdf', format='pdf', bbox_inches="tight", pad_inches=0.2)
