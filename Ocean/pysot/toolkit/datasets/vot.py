...
class VOTVideo(Video):
    ...
    def load_tracker(self, path, tracker_names=None, store=True, model_epoch=None, expcase=None, trajcase=None):
        """
        Args:
        ...
        for name in tracker_names:
            ...
            if trajcase is None:
                traj_files = glob(os.path.join(path, name, 'baseline', str(
                    expcase), model_epoch, self.name, '*0*.txt'))
                # print(traj_files)
                # exit()

            else:
                traj_files = glob(os.path.join(path, name, 'baseline', str(expcase),
                                               model_epoch, str(trajcase), self.name, '*01_pred.txt'))
                # print(traj_files)
                # exit()
...
class VOTLTVideo(Video):
    ...
    def load_tracker(self, path, tracker_names=None, store=True, model_epoch=None, expcase=None, trajcase=None):
        ...
        for name in tracker_names:
            ...
            if trajcase is None:
                traj_file = os.path.join(path, name, 'longterm', str(
                    expcase), model_epoch, self.name, self.name + '_001.txt')

            else:
                traj_file = os.path.join(path, name, 'longterm', str(
                    expcase), model_epoch, str(trajcase), self.name, self.name + '_001_pred.txt')
            ...
            if trajcase is None:
                confidence_file = os.path.join(path, name, 'longterm', str(expcase), model_epoch,
                                               self.name, self.name + '_001_confidence.value')

            else:

                confidence_file = os.path.join(path, name, 'longterm', str(expcase), model_epoch, str(trajcase),
                                               self.name, self.name + '_001_confidence.value')
