        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                # print((gt_traj))
                    ...
                    gt_traj_file = os.path.join(self.dataset.tracker_path, tracker_name,
                                                str(self.expcase), self.model_epoch, str(self.trajcase), video.name + '_target.txt')
                ...
                if tracker_name not in video.pred_trajs:
                    ...
                    # ADDED THIS SINCE missed to code first frame for target. CHANGED
    ...
    def eval_norm_precision(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if tracker_name not in video.pred_trajs:
                    tracker_traj = video.load_tracker(self.dataset.tracker_path,
                                                      tracker_name, False, self.model_epoch, self.expcase, str(
                                                          self.trajcase))
                    tracker_traj = np.array(tracker_traj)
    ...
    def show_result(self, success_ret, precision_ret=None,
        ...
        for tracker_name in tracker_names:
            ...
            if precision_ret is not None:
                precision = np.mean(list(precision_ret[tracker_name].values()), axis=0)[20]    # CHANGED
            else:
                precision = 0
