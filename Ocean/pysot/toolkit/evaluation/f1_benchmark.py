...
class F1Benchmark:
    def __init__(self, dataset, model_epoch=None, expcase=None):
        """
        Args:
            result_path:
        """
        ...
        for video in self.dataset:
            if tracker_name not in video.confidence:
                score += video.load_tracker(self.dataset.tracker_path, tracker_name,
                                            False, self.model_epoch, self.expcase)[1]
            else:
        ...
        for i in range(len(self.dataset)):
            ...
            if tracker_name not in video.pred_trajs:
                tracker_traj, score = video.load_tracker(
                    self.dataset.tracker_path, tracker_name, False, self.model_epoch, self.expcase)
            else:
