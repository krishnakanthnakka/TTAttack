...
python ttattack_untargeted.py --dataset=OTB100  --tracker_name=siam_ocean_online --case=1  --model_iter=4_net_G.pth --gpu=0 --trajcase=0 --attack_universal --video=Biker --vis
...
def main(cmd_line):
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            log('({:3d}) Video: {:12s} Time: {:4.1f}s Speed: {:3.1f}fps Lost: {:d}, '.format(
        ...
        for v_idx, video in enumerate(dataset):
            ...
            log('({:3d}) Video: {:12s} Time: {:5.1f}s Speed: {:3.1f}fps, mean Speed: {:3.1f},  Lost:{:4d}'.format(
                v_idx + 1, video.name, toc, idx / toc, np.mean(mean_FPS), outputs['lost']))
